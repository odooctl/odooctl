...
> **Local-only folder** (since 2026-07-21): `docs/plans/` is untracked and
> purged from git history — it exists only on this machine. Plans are numbered
> `m##`; the highest number is the newest. Progress lives in `progress.md`
> (compact) and `final-run-progress.md` (m16 roadmap tracker).
...
11. `m16-roadmap-2026-07-production-readiness.md` — 1.0 GA production-readiness roadmap (tracker: `final-run-progress.md`).
12. `m17-feedback-enhancement-plan.md` — Reddit-feedback steers: neutralize-first sanitize, sync/auto-deploy, config overlay, identity/RBAC, storage backends.
