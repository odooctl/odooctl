# Odoo 18 full-stack test report
...
```bash
ODOOCTL_IT_VERSIONS=18.0 uv run pytest -m integration tests/integration -q -ra
```

Result: **7 passed, 4 opt-in tests skipped in 126.42 seconds**.
...
Odoo 18 database, filestore, sanitization, restore, API/runner, and Compose lifecycle behavior passes. The only supplemental failure is a shared PITR test-fixture validation error, not an Odoo 18 runtime failure.
